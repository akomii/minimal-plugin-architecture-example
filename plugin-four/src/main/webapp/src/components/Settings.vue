  ...
  <div class="grid">
    <div class="col-12">
      <Card>
        ...
        <template #subtitle>
          Discover frontend ES modules exposed by backend-loaded plugin JARs and activate or deactivate them in the browser.
        </template>
        ...
        <template #content>
          <div class="flex justify-content-between align-items-center mb-4 gap-3 flex-wrap">
            ...
            <Button
                label="Refresh"
                icon="pi pi-refresh"
                :loading="isRefreshingPlugins"
                @click="handleRefresh"
            />
          </div>

          <Message v-if="pluginError" severity="error" class="mb-4">
            {{ pluginError }}
          </Message>

          <Message v-if="!pluginError && availablePlugins.length === 0" severity="warn" class="mb-4">
            No frontend plugins are currently available from the backend.
          </Message>
          ...
          <div v-if="availablePlugins.length > 0" class="flex flex-column gap-3">
            ...
            >
              <div>
                ...
                <div class="text-sm text-600">ID: {{ plugin.id }}</div>
                <div class="text-sm text-600">Route: {{ plugin.path }}</div>
              ...
              <div class="flex align-items-center gap-3">
                ...
                <ToggleSwitch
                    :model-value="isPluginActive(plugin.id)"
